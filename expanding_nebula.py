...
def solution(g, a=0, b=0, previous=0, solutions=0, history=0):
	...
	for cell in [True, False]:
		if (not a or not b) or len(set([((previous[a][b-1] + previous[a-1][b] + previous[a-1][b-1] + cell)==1), g[a-1][b-1]]))==1:
			history.append(cell)
			previous[a][b] = cell
			res += solution(g, a=(a+1)%(len(g)+1), b=b+(a+1)//(len(g)+1), previous=previous, solutions=solutions, history=history)
			history.pop()
